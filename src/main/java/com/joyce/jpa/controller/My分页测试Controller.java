...
public class My分页测试Controller {
    ...
    @ResponseBody
    public Page<EmployeeModel> 分页方式1() {
        Page<EmployeeModel> page = jpa的4种分页Service.分页方式1_自己查询总数自己查询数据最后组装分页对象();
        return page;
    }

    @RequestMapping("/fenye-2") // http://localhost:8080/fenye-2?id=60
    @ResponseBody
    public Page<EmployeeModel> 分页方式2(Integer id) {
        Page<EmployeeModel> page = jpa的4种分页Service.分页方式2_使用jpa的pageable进行分页(id);
        return page;
    }

    @RequestMapping("/fenye-3") // http://localhost:8080/fenye-3?page=0&size=5
    @ResponseBody
    public Page<EmployeeModel> 分页方式3(Integer page, Integer size) {
        Page<EmployeeModel> list = jpa的4种分页Service.分页方式3_jpa已经实现的分页方式(page, size);
        return list;
    }

    @RequestMapping("/fenye-4") // http://localhost:8080/fenye-4?page=0&size=5&id=2
    @ResponseBody
    public Page<EmployeeModel> 分页方式4(Integer id, Integer page, Integer size) {
        Page<EmployeeModel> list = jpa的4种分页Service.分页方式4_自定义分页SQL和总数查询countQuery(id, page, size);
        return list;
    }
